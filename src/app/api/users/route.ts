...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log('Creating user with data:', { ...userToCreate, password: '[HIDDEN]' });
    ...
    return NextResponse.json(
      { success: false, error: `Failed to create user: ${error instanceof Error ? error.message : 'Unknown error'}` },
      { status: 500 }
    );
  }
}
